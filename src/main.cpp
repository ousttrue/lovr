...
static int luaopen_lovr_nogame(lua_State *L) {
  if (!luaL_loadbuffer(L, (const char *)etc_nogame_lua, etc_nogame_lua_len,
                       "@nogame.lua")) {
    lua_call(L, 0, 1);
  }
...
static std::vector<char> read_all(const char *file) {
  std::ifstream t(file);
  if (!t) {
    return {};
  }
...
int main(int argc, char **argv) {
  if (argc > 1 && (!strcmp(argv[1], "--version") || !strcmp(argv[1], "-v"))) {
    ...
    printf("LOVR %d.%d.%d (%s)\n", LOVR_VERSION_MAJOR, LOVR_VERSION_MINOR,
           LOVR_VERSION_PATCH, LOVR_VERSION_ALIAS);
    exit(0);
  }

  if (argc > 1 && (!strcmp(argv[1], "--help") || !strcmp(argv[1], "-h"))) {
    os_open_console();
    printf("usage: lovr [options] [<source>]\n\n"
           "options:\n"
  ...
  do {
    ...
    const luaL_Reg nogame[] = {{"nogame", luaopen_lovr_nogame}, {NULL, NULL}};
    ...
    // Now that we know the negative offset to start at, copy all args in the
    // table
    ...
    auto p = (const char *)etc_boot_lua;
    auto len = etc_boot_lua_len;
    auto name = "@boot.lua";
    auto read_boot_lua = read_all("boot.lua");
    if (!read_boot_lua.empty()) {
      // read boot.lua from cwd.
      p = read_boot_lua.data();
      len = read_boot_lua.size();
      name = "@boot.lua(read)";
    }

    lua_pushcfunction(L, luax_getstack);
    if (luaL_loadbuffer(L, p, len, name) || lua_pcall(L, 0, 1, -2)) {
      fprintf(stderr, "%s\n", lua_tostring(L, -1));
      return 1;
    }
    ...
    restart =
        lua_type(T, 1) == LUA_TSTRING && !strcmp(lua_tostring(T, 1), "restart");
